 ...
 * @author Pablo
 */
public class BajadaSueldoController implements Initializable {
    ...
    @FXML
    private void aplicarBajadaSueldo(ActionEvent event) {
        String posibleSubida = this.txtfDNI.getText();
        boolean existeDNI = false;
        for (Vendedor posible : vendedores) {
            if (posible.getDNI().equals(posibleSubida)) {
                existeDNI = true;
                posible.bajarSueldo(Double.parseDouble(this.txtfVariacion.getText()));
