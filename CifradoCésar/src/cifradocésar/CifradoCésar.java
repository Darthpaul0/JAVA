...
public class CifradoCésar {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            // Antes de leer hay que comprobar que el tipo de valor introducido se
            // corresponde con el solicitado
            // Si no lo hacemos y leemos directamente un valor no válido se lanzaría la
    ...
    public static String Cifrador() {
        ...
        for (int i = 0; i < mensaje.length(); i++) { // recorremos el mensaje original
            ...
            if (letra >= 0) {
                cifrado += abecedario.charAt((letra + k) % abecedario.length()); // el módulo se utiliza en caso de que
                                                                                 // al cifrar sobrepasemos la última
                                                                                 // letra del alfabeto
            } else {
    ...
    public static String Descifrador() {
        ...
        for (int i = 0; i < mensaje.length(); i++) { // recorremos el mensaje original
            letra = abecedario.indexOf(mensaje.charAt(i)); // calculamos la posición de las letras en el abecedario
            if (letra >= 0) {
                descifrado += abecedario.charAt(Math.abs((letra - k)) % abecedario.length()); // el módulo se utiliza en
                                                                                              // caso de que al cifrar
                                                                                              // sobrepasemos la última
                                                                                              // letra del alfabeto
            } else {
    ...
    public static String FuerzaBrutaCesar() {
        ...
        while (k > 0 && k <= 27) {
            for (int i = 0; i < mensaje.length(); i++) { // recorremos el mensaje original
                letra = abecedario.indexOf(mensaje.charAt(i)); // calculamos la posición de las letras en el abecedario
                if (letra >= 0) {
                    descifrado += abecedario.charAt(Math.abs((letra - k + abecedario.length())) % abecedario.length()); // el
                                                                                                                        // módulo
                                                                                                                        // se
                                                                                                                        // utiliza
                                                                                                                        // en
